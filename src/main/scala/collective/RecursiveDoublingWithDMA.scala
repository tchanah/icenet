...
class RecursiveDoublingWithDMAModuleImp(outer: RecursiveDoublingWithDMA) extends LazyModuleImp(outer) {
  ...
  val s_idle :: s_recv_data :: s_write_buffer :: s_wait_write_buffer :: s_process :: s_dma_write :: s_wait_write :: s_dma_read :: s_wait_read :: s_wait_read_done :: s_send_meta :: s_send_data :: s_check_buffer :: Nil = Enum(13)
  ...
  .elsewhen(state === s_wait_read_done) {
      ...
      when(isReadingBufferedPacket) {
          ...
          // Perform element-wise addition for recursive processing
          dprintf(p"[s_wait_read_done] Start addition for level ${currentLevelReg}\n")
          dprintf(p"[s_wait_read_done] Before addition: processedDataBuffer[255]=0x${Hexadecimal(processedDataBuffer(255))}\n")
          dprintf(p"[s_wait_read_done] incomingDataBuffer[255]=0x${Hexadecimal(incomingDataBuffer(255))}, memoryReadBuffer[255]=0x${Hexadecimal(memoryReadBuffer(255))}\n")
          for (i <- 0 until NUM_DATA_ELEMENTS) {
              val incomingVal = incomingDataBuffer(i)
              val memoryVal = memoryReadBuffer(i)
              val sum = incomingVal + memoryVal
              processedData(i) := sum
              when(storeData) {
                  processedDataBuffer(i) := sum
                  when(i.U === 255.U) {
                      dprintf(p"[s_wait_read_done] Writing processedDataBuffer[255] = 0x${Hexadecimal(sum)} (incoming=0x${Hexadecimal(incomingVal)} + memory=0x${Hexadecimal(memoryVal)})\n")
                  }
              }
          }

          when(storeData) {
              dprintf(p"[s_wait_read_done] Store processed data to memory for level ${currentLevelReg}\n")
              dprintf(p"[s_wait_read_done] processedDataBuffer[0]=0x${Hexadecimal(processedDataBuffer(0))}, processedDataBuffer[255]=0x${Hexadecimal(processedDataBuffer(255))}\n")
              state := s_dma_write
              dmaWordCount := 0.U
          }.otherwise {
              dprintf(p"[s_wait_read_done] Send processed data directly (no storage)\n")
              state := s_send_meta
              sentWordCount := 0.U
          }
      }
